...
# @file           bindings/python/test/coordinate/frame/providers/iers/test_finals_2000a.py
...
class TestFinals2000A:
    ...
    def test_get_interval_success (self, finals_2000a: Finals2000A):
        ...
        assert finals_2000a.get_interval().to_string() == '[1992-01-01 00:00:00 - 2021-12-26 00:00:00] [UTC]'

    def test_get_polar_motion_at_success (self, finals_2000a: Finals2000A):

        assert list(finals_2000a.get_polar_motion_at(Instant.date_time(datetime(2020, 10, 31, 0, 0, 0), Scale.UTC))) == [0.160621, 0.297128]

    def test_get_ut1_minus_utc_at_success (self, finals_2000a: Finals2000A):

        assert finals_2000a.get_ut1_minus_utc_at(Instant.date_time(datetime(2020, 10, 31, 0, 0, 0), Scale.UTC)) == -0.17524010000000001

    def test_get_lod_at_success (self, finals_2000a: Finals2000A):

        assert finals_2000a.get_lod_at(Instant.date_time(datetime(2015, 10, 31, 0, 0, 0), Scale.UTC)) == 1.5646
        assert not finals_2000a.get_lod_at(Instant.date_time(datetime(2020, 10, 31, 0, 0, 0), Scale.UTC)).is_defined()

    def test_get_data_at_success (self, finals_2000a: Finals2000A):

        data: Finals2000A.Data = finals_2000a.get_data_at(Instant.date_time(datetime(2020, 10, 31, 0, 0, 0), Scale.UTC))
