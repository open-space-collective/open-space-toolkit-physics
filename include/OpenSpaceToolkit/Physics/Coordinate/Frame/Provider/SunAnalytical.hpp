...
#ifndef __OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_SunAnalytical__
#define __OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_SunAnalytical__
...
/// @brief Low-precision analytical Sun frame provider.
///
/// Provides the transform from GCRF to a frame centered on the Sun, with an identity orientation
/// with respect to GCRF. The Sun position is computed from the analytical series of Montenbruck & Gill,
...
/// < 0.1 deg in direction, < 0.01% in distance.
/// The dominant error is a slow drift (~11.6 arcsec/year) due to the neglected motion of the Earth's perihelion.
///
...
class SunAnalytical : public Provider
{
   public:
    /// @brief Constructor
    SunAnalytical();
    ...
    /// @brief Destructor
    virtual ~SunAnalytical() override;

    /// @brief Clone
    ///
    /// @return Pointer to SunAnalytical
    virtual SunAnalytical* clone() const override;

    /// @brief Returns true if SunAnalytical is defined
    ///
    /// @return True if SunAnalytical is defined
