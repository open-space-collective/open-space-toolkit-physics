...
\DoxyCodeLine{export OSTK\_DATA\_LOCAL\_CACHE=/path/to/preferred/data/storage/dir}
\DoxyCodeLine{export OSTK\_PHYSICS\_DATA\_LOCAL\_REPOSITORY=\$OSTK\_DATA\_LOCAL\_CACHE/data}
\DoxyCodeLine{}
\DoxyCodeLine{\# Clone OSTk data repo into that dir}
\DoxyCodeLine{RUN git clone -\/-\/branch v1 -\/-\/single-\/branch -\/-\/depth=1 https://github.com/open-\/space-\/collective/open-\/space-\/toolkit-\/data.git \$OSTK\_DATA\_LOCAL\_CACHE \&\& chmod -\/R g+w \$OSTK\_DATA\_LOCAL\_CACHE}
\end{DoxyCode}


For users that are installing O\+S\+Tk into docker images, here is what we recommend you add to your dockerfiles. 
\begin{DoxyCode}{0}
\DoxyCodeLine{ARG OSTK\_DATA\_LOCAL\_CACHE="/var/cache/open-\/space-\/toolkit-\/data"}
\DoxyCodeLine{ENV OSTK\_DATA\_LOCAL\_CACHE="\$\{OSTK\_DATA\_LOCAL\_CACHE\}"}
\DoxyCodeLine{ENV OSTK\_PHYSICS\_DATA\_LOCAL\_REPOSITORY="\$\{OSTK\_DATA\_LOCAL\_CACHE\}/data"}
\DoxyCodeLine{}
\DoxyCodeLine{RUN git clone -\/-\/branch v1 -\/-\/single-\/branch -\/-\/depth=1 https://github.com/open-\/space-\/collective/open-\/space-\/toolkit-\/data.git \$\{OSTK\_DATA\_LOCAL\_CACHE\} \&\& chmod -\/R g+w \$\{OSTK\_DATA\_LOCAL\_CACHE\}}
\DoxyCodeLine{}
\DoxyCodeLine{\# Add the line below if the container's user will not be root}
\DoxyCodeLine{RUN chown -\/R \$\{USER\_UID\}:\$\{USER\_GID\} \$\{OSTK\_DATA\_LOCAL\_CACHE\}}
\end{DoxyCode}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\# If you are copying to an image where the user is currently root}
\DoxyCodeLine{COPY -\/-\/from=build-\/env \$\{OSTK\_DATA\_LOCAL\_CACHE\} \$\{OSTK\_DATA\_LOCAL\_CACHE\}}
\DoxyCodeLine{\# Or if you are copying to an image where the user is not currently root}
\DoxyCodeLine{COPY -\/-\/from=build-\/env -\/-\/chown=\$\{USER\_UID\}:\$\{USER\_GID\} \$\{OSTK\_DATA\_LOCAL\_CACHE\} \$\{OSTK\_DATA\_LOCAL\_CACHE\}}
\end{DoxyCode}
