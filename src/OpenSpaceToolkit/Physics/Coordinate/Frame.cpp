...
bool Frame::operator==(const Frame& aFrame) const
{
    ...
    return (name_ == aFrame.name_) && (quasiInertial_ == aFrame.quasiInertial_) &&
           (((parentFrameSPtr_ == nullptr) && (aFrame.parentFrameSPtr_ == nullptr)) ||
            ((parentFrameSPtr_ != nullptr) && (aFrame.parentFrameSPtr_ != nullptr) &&
             ((*parentFrameSPtr_) == (*aFrame.parentFrameSPtr_)))) &&
           ((providerSPtr_ != nullptr) && (aFrame.providerSPtr_ != nullptr) &&
            (providerSPtr_.get() == aFrame.providerSPtr_.get()));
}
...
std::ostream& operator<<(std::ostream& anOutputStream, const Frame& aFrame)
{
    ...
    ostk::core::utils::Print::Line(anOutputStream)
        << "Quasi-inertial:" << (aFrame.isDefined() ? String::Format("{}", aFrame.isQuasiInertial()) : "Undefined");
    ostk::core::utils::Print::Line(anOutputStream)
        << "Parent frame:"
        << (((aFrame.parentFrameSPtr_ != nullptr) && (aFrame.parentFrameSPtr_->isDefined()))
                ? aFrame.parentFrameSPtr_->getName()
                : "None");
...
Shared<const Frame> Frame::accessAncestor(const Uint8 anAncestorDegree) const
{
    ...
    if (anAncestorDegree > this->getDepth())
    {
        throw ostk::core::error::RuntimeError(
            "Ancestor degree [{}] is greater than depth [{}].", anAncestorDegree, this->getDepth()
        );
    }
...
Position Frame::getOriginIn(const Shared<const Frame>& aFrameSPtr, const Instant& anInstant) const
{
...
Velocity Frame::getVelocityIn(const Shared<const Frame>& aFrameSPtr, const Instant& anInstant) const
{
    if (!anInstant.isDefined())
    {
        throw ostk::core::error::runtime::Undefined("Instant");
...
Transform Frame::getTransformTo(const Shared<const Frame>& aFrameSPtr, const Instant& anInstant) const
{
    if (!anInstant.isDefined())
    {
        throw ostk::core::error::runtime::Undefined("Instant");
    ...
    {
        throw ostk::core::error::RuntimeError(
            "No common ancestor between [{}] and [{}].", this->getName(), aFrameSPtr->getName()
        );
    }
...
Shared<const Frame> Frame::J2000(const iau::Theory& aTheory)
{
    ...
    return Frame::Emplace(
        String::Format("J2000 ({})", iau::StringFromTheory(aTheory)), true, Frame::GCRF(), providerSPtr
    );
}
...
Shared<const Frame> Frame::TOD(const Instant& anEpoch, const iau::Theory& aTheory)
{
    using Scale = ostk::physics::time::Scale;
...
Shared<const Frame> Frame::TEMEOfEpoch(const Instant& anEpoch)
{
    ...
    const Shared<const Provider> providerSPtr =
        std::make_shared<const StaticProvider>(Frame::GCRF()->getTransformTo(Frame::TEME(), anEpoch));
...
Shared<const Frame> Frame::Construct(
    const String& aName,
    bool isQuasiInertial,
    const Shared<const Frame>& aParentFrame,
    const Shared<const Provider>& aProvider
...
Shared<const Frame> Frame::Emplace(
    const String& aName,
    bool isQuasiInertial,
    const Shared<const Frame>& aParentFrame,
    const Shared<const Provider>& aProvider
)
{
    ...
    const Shared<const Frame> frameSPtr =
        std::make_shared<const SharedFrameEnabler>(aName, isQuasiInertial, aParentFrame, aProvider);
...
Shared<const Frame> Frame::FindCommonAncestor(
...
)
{
    ...
    Shared<const Frame> currentFSPtr = (firstFrameDepth > secondFrameDepth)
                                         ? aFirstFrameSPtr->accessAncestor(firstFrameDepth - secondFrameDepth)
                                         : aFirstFrameSPtr;
    Shared<const Frame> currentTSPtr = (firstFrameDepth > secondFrameDepth)
                                         ? aSecondFrameSPtr
                                         : aSecondFrameSPtr->accessAncestor(secondFrameDepth - firstFrameDepth);
