...
void NRLMSISE00::computeNRLMSISE00Input(nrlmsise_input& input, ap_array& aph, const LLA& aLLA, const Instant& anInstant, const Position& sunPosition) const
{
    ...
    if (sunPosition.isDefined())
    {
        lst = (Real::Pi() + std::atan2(
            sunPosition.accessCoordinates()[0] * position.accessCoordinates()[1] - sunPosition.accessCoordinates()[1] * position.accessCoordinates()[0],
            sunPosition.accessCoordinates()[0] * position.accessCoordinates()[0] + sunPosition.accessCoordinates()[1] * position.accessCoordinates()[1])) * 12.0 / Real::Pi();
    } else {
