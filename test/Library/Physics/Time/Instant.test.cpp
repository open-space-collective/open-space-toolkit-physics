...
TEST (Library_Physics_Time_Instant, Constructor)
{
...

    {
        ...
        const Uint64 nanosecondCountFromEpoch = 123456 ;
        const bool postEpoch = true ;
        const Scale timeScale = Scale::UTC ;

        EXPECT_NO_THROW(Instant(nanosecondCountFromEpoch, postEpoch, timeScale)) ;

    }
    
    {

        const Uint64 nanosecondCountFromEpoch = 0 ;
        const bool postEpoch = false ;
        const Scale timeScale = Scale::TAI ;
...

    {
        ...
        const Uint64 nanosecondCountFromEpoch = 0 ;
        const bool postEpoch = true ;
        const Scale timeScale = Scale::Undefined ;
...
TEST (Library_Physics_Time_Instant, Epoch)
{
...

    {
        ...
        EXPECT_NO_THROW(Instant::Epoch()) ;
        EXPECT_TRUE(Instant::Epoch().isDefined()) ;

        EXPECT_EQ(Instant(0, true, Scale::TT), Instant::Epoch()) ;
