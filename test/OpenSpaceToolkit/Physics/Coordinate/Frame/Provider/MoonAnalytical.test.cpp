...
using ostk::physics::coordinate::frame::provider::MoonAnalytical;
...
class OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_MoonAnalytical : public ::testing::Test
{
   protected:
    Engine& engine_;
    Manager& manager_;

    OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_MoonAnalytical()
        : engine_(Engine::Get()),
...
TEST_F(OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_MoonAnalytical, IsDefined)
{
    {
        EXPECT_TRUE(MoonAnalytical().isDefined());
    }
}

TEST_F(OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_MoonAnalytical, GetTransformAt)
{
    {
        const Instant instant = Instant::DateTime(DateTime(2024, 1, 1, 0, 0, 0), Scale::UTC);

        EXPECT_TRUE(MoonAnalytical().getTransformAt(instant).isDefined());
    }

    {
        EXPECT_ANY_THROW(MoonAnalytical().getTransformAt(Instant::Undefined()));
    }
}

TEST_F(OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_MoonAnalytical, PositionVersusSpice)
{
    {
        this->loadSpiceKernels();

        const Shared<const Frame> analyticalFrameSPtr = Frame::MoonAnalytical();
...
TEST_F(OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_MoonAnalytical, VelocityVersusSpice)
{
    {
        this->loadSpiceKernels();
        ...
        for (const auto& instant : ComparisonInstants())
        {
            ...
            const Vector3d v_GCRF_spice = moonSpice.getVelocityIn(Frame::GCRF(), instant).getCoordinates();
