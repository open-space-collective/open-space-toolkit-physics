...
using ostk::physics::coordinate::frame::provider::SunAnalytical;
...
class OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_SunAnalytical : public ::testing::Test
{
   protected:
    Engine& engine_;
    Manager& manager_;

    OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_SunAnalytical()
        : engine_(Engine::Get()),
...
TEST_F(OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_SunAnalytical, IsDefined)
{
    {
        EXPECT_TRUE(SunAnalytical().isDefined());
    }
}

TEST_F(OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_SunAnalytical, GetTransformAt)
{
    {
        const Instant instant = Instant::DateTime(DateTime(2024, 1, 1, 0, 0, 0), Scale::UTC);

        EXPECT_TRUE(SunAnalytical().getTransformAt(instant).isDefined());
    }

    {
        EXPECT_ANY_THROW(SunAnalytical().getTransformAt(Instant::Undefined()));
    }
}

TEST_F(OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_SunAnalytical, PositionVersusSpice)
{
    {
        this->loadSpiceKernels();

        const Shared<const Frame> analyticalFrameSPtr = Frame::SunAnalytical();
...
TEST_F(OpenSpaceToolkit_Physics_Coordinate_Frame_Provider_SunAnalytical, VelocityVersusSpice)
{
    {
        this->loadSpiceKernels();
