...
TEST (OpenSpaceToolkit_Physics_Coordinate_Frame_Providers_TEME, GetTransformAt)
{
...

    {
        ...
        {
            { File::Path(Path::Parse("/app/test/OpenSpaceToolkit/Physics/Coordinate/Frame/Providers/TEME/Scenario_A/Scenario ITRF_TEME.csv")), Angle::Arcseconds(1.0), 1e-9 },
            { File::Path(Path::Parse("/app/test/OpenSpaceToolkit/Physics/Coordinate/Frame/Providers/TEME/Scenario_B/Scenario ITRF_TEME.csv")), Angle::Arcseconds(20.0), 1e-9 }
        } ;
        ...
        for (const auto& referenceScenario : referenceScenarios)
        {
            ...
            for (const auto& referenceRow : referenceData)
            {
                ...
                const Quaternion referenceOrientation_GCRF_TEME = Quaternion::XYZS(referenceRow[9].accessReal(), referenceRow[10].accessReal(), referenceRow[11].accessReal(), referenceRow[12].accessReal()).normalize() ;
