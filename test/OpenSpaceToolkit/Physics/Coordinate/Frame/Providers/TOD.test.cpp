...
TEST (OpenSpaceToolkit_Physics_Coordinate_Frame_Providers_TOD, GetTransformAt)
{
...

    {
        ...
        const Quaternion ref_q_J2000_TOD = Quaternion::XYZS(1.58621719623e-05, -1.10716940847e-03, 2.54801869031e-03, 9.99996140755e-01).toNormalized() ;

        const Quaternion q_J2000_TOD_IAU_2000 = Frame::TOD(instant, Theory::IAU_2000B)->getTransformTo(Frame::J2000(Theory::IAU_2000A), instant).getOrientation() ;
        const Quaternion q_J2000_TOD_IAU_2006 = Frame::TOD(instant, Theory::IAU_2006)->getTransformTo(Frame::J2000(Theory::IAU_2006), instant).getOrientation() ;

        EXPECT_GT(1.0, q_J2000_TOD_IAU_2000.angularDifferenceWith(ref_q_J2000_TOD).inRadians() * Earth::EquatorialRadius.inMeters()) ;
        EXPECT_GT(1.0, q_J2000_TOD_IAU_2006.angularDifferenceWith(ref_q_J2000_TOD).inRadians() * Earth::EquatorialRadius.inMeters()) ;
